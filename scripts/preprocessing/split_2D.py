...
def split_cols_idx(dataset, splits, test_size = 0.25, limit = 1):
    """
...
def split_slices_idx(dataset, test_size = 0.25, limit = 1):
...
def test_train_split(dataset, fail_times, splits, mode, test_size = 0.25,
    ...
    if mode == 'col':
        dummy1, dummy2 = split_cols_idx(dataset, splits, test_size, limit)
    elif mode == 'slice':
        dummy1, dummy2 = split_slices_idx(dataset, test_size, limit)
    ...
    for m, sample in enumerate(dataset):
        ...
        if mode == 'col':
            ...
            split_cols_idx(dataset, splits, test_size, limit)
        elif mode == 'slice':
            indices_train, indices_test = \
            split_slices_idx(dataset, test_size, limit)
        ...
        if X_train is None:
            ...
            X_train = np.zeros((shape1, shape2[0], shape2[1], shape2[2]))
            y_train = np.zeros((shape1))

            shape1 = len(indices_test)*len(dataset)
            idx_test = indices_train[0]
            shape2 = sample[idx_test[0], idx_test[1], idx_test[2]].shape

            X_test = np.zeros((shape1, shape2[0], shape2[1], shape2[2]))
            y_test = np.zeros((shape1))
...
if __name__ == "__main__":
    ...
    splits = 100
    limit = 0.01
    X_train, X_test, y_train, y_test = test_train_split(dataset, fail_times, splits, "col", limit=limit)
