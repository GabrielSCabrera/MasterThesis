...
def split_cols_idx(dataset, splits, test_size = 0.25, limit = 1, shuffle = True):
    ...
    """
    dims = dataset.dims
    splits = int(splits/limit)
    step = max(int(1/limit), 1)

    x_idx_max = (dims[0]//splits+1)*splits
    x_idx = np.arange(0, x_idx_max, splits, dtype = np.int64)

    y_idx = np.arange(0, dims[1], dtype = np.int64)

    x_idx = np.array([slice(i,j,step) for i,j in zip(x_idx[:-1], x_idx[1:])])
    y_idx = np.array([slice(i,j) for i,j in zip(y_idx[:-1], y_idx[1:])])
...
def split_slices_idx(dataset, splits, test_size = 0.25, limit = 1, shuffle = True):
    ...
    z_idx = np.arange(0, dims[2]+1, dtype = np.int64)
