...
def combine_logspace_results(experiment:str, directory:Path = None) -> None:
    '''
        Takes results from a set of delvol logspace experiments saved in a
        folder of format `/logspace_YYYY-MM-DD HH:MM:SS.XXXXXX`, which are by
        default saved to `~/Documents/MasterThesis/results/delvol`.
    '''
    if directory is None:
        directory = config.delvol_relpath
    ...
    for i in temp:
        if i.is_dir():
            experiments_unsorted.append(i)

    numbers = []
    for i in experiments_unsorted:
        N = int(i.name.split('_')[-1][1:])
        numbers.append(N)

    idx = np.argsort(numbers)
    experiments = []
    for i in idx:
        experiments.append(experiments_unsorted[i])
...
def combine_linspace_results(experiment:str, directory:Path = None) -> None:
    '''
        Takes results from a set of delvol linspace experiments saved in a
        folder of format `/linspace_YYYY-MM-DD HH:MM:SS.XXXXXX`, which are by
        default saved to `~/Documents/MasterThesis/results/delvol`.
    '''
    if directory is None:
        directory = config.delvol_relpath
